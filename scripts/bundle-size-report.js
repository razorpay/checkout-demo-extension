 ...
 *   async: { def-123.js: { base, pr, diff, diff% } }
 * }
 */
function calculateDifferences(baseStats, prStats) {
  ...
  const asyncChunks = Array.from(
    new Set([
      ...Object.keys(baseStats.async).concat(Object.keys(prStats.async)),
    ])
  ).sort();
  ...
  const report = {
    ...
    async: Object.fromEntries(
      asyncChunks.map((chunkName) => [
        chunkName,
        extractStatsForChunk(chunkName, baseStats.async, prStats.async),
      ])
    ),
  };
...
function splitByFormat(fullPath, stats, file) {
  ...
  if (
    fullPath.endsWith('.gz') || // we take care of gzip separately below
    fullPath.includes('-1cc.js') ||
    fullPath.endsWith('.map') ||
    fullPath.endsWith('-new.js')
  ) {
    return stats;
  }

  const isChunk =
    /.*\-\d\d\d\.(js|css)/g.test(file) || // js chunk -> abc-123.js
    /\d\d\d\..*/g.test(file); // css chunk -> 123.abc.css

  const parentKey = isChunk ? 'async' : 'initial';

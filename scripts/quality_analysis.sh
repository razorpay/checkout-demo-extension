...
# defining globals
MASTER_PROJECT="CheckoutCoverageCheck"
DEV_PROJECT="CheckoutDevCoverageCheck"
...
then
  ...
  # get dev metrics - new code coverage
  CURL_URL=$MEASURES_COMPONENT_TREE_API'?metricKeys=new_coverage&component='$DEV_PROJECT
  curl --location --request GET "$CURL_URL" -u $SONAR_TOKEN:"" > sonar.json
  DEV_COVERAGE_ON_NEW_CODE=$(jq '.baseComponent.measures | .[].value' sonar.json)
  echo ::set-output name=dev_coverage_on_new_code::"$DEV_COVERAGE_ON_NEW_CODE"
else
  echo "This is an older commit as per sonarqube"

  # get dev project PR metrics
  CURL_URL=$MEASURES_SEARCH_HISTORY_API'?component='$DEV_PROJECT'&metrics=duplicated_lines_density,coverage,bugs,code_smells,vulnerabilities,new_coverage&ps=1000&p=1'
  curl --location --request GET "$CURL_URL" -u $SONAR_TOKEN:"" > sonar.json
  ...
  # extract new code coverage
  DEV_COVERAGE_ON_NEW_CODE=$(jq '.measures | .[] | select(.metric=="new_coverage").history | .[] | select(.date=='"$PR_DATE"').value' sonar.json)
  echo ::set-output name=dev_coverage_on_new_code::"$DEV_COVERAGE_ON_NEW_CODE"
fi
...
DEV_COVERAGE_ON_NEW_CODE=$(echo "$DEV_COVERAGE_ON_NEW_CODE" | bc)
...
PASSED=$( cut -d "," -f 3 ${GITHUB_WORKSPACE}/utMetrics.csv)
FAILED=$( cut -d "," -f 4 ${GITHUB_WORKSPACE}/utMetrics.csv)
SKIPPED=$( cut -d "," -f 5 ${GITHUB_WORKSPACE}/utMetrics.csv)

echo "Start Time: ${START_TIME}"
echo "End Time: ${END_TIME}"
echo "Test cases Passed: ${PASSED}"
echo "Test cases Skipped: ${FAILED}"
echo "Test cases Failed: ${SKIPPED}"

echo "INSERT INTO UTCoverage VALUES (\"CHECKOUT\",\"${GIT_SHA}\",\"${BRANCH}\",\"Payments\",\"{\\\"tags\\\": [\\\"SME\\\"]}\",\"${DEV_CODE_COVERAGE}\",\"${START_TIME}\",\"${END_TIME}\",\"${PASSED}\",\"${FAILED}\",\"${SKIPPED}\");"

curl -X POST \
       https://mock-go.qa.razorpay.in/insert_qa_iteration \
       -H "content-type: text/plain" \
       -d "INSERT INTO UTCoverage VALUES (\"CHECKOUT\",\"${GIT_SHA}\",\"${BRANCH}\",\"Payments\",\"{\\\"tags\\\": [\\\"SME\\\"]}\",\"${DEV_CODE_COVERAGE}\",\"${START_TIME}\",\"${END_TIME}\",\"${PASSED}\",\"${FAILED}\",\"${SKIPPED}\");"
...
echo ::set-output name=quality_report_result::"$RESULT"

<script>
  ...
  let AVSInfo = [];
  // experiments
  import { delayLoginOTPExperiment, updateAVSScreenMap } from 'card/helper';
  ...
  const getCardByTokenId = (tokenId) => {
    if (!tokenId) {
      return;
    }
    if (!$customer.tokens) {
  ...
  const fetchAVSFlag = (params = {}) => {
    const key = params.iin || params.tokenId;
    if (key) {
      getCurrencies(params).then((currencyPayload) => {

...
// move this to another file and import here if more optimisations are needed.
const optimiseInstruments = instruments => {
  // divide array into two splits based on a validity condition
  ...
  // Optimisation: Merge all upi intent instruments which have the same package name into a single instrument
...
export function setInstrumentsForCustomer(
  ...
  const optimisedInstruments = optimiseInstruments(transformedInstruments);

  PREFERRED_INSTRUMENTS_CACHE[customer.contact] = Promise.resolve({
    identified,
    instruments: optimisedInstruments,
  });

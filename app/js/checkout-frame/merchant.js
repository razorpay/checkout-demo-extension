...
function setPaymentMethods(session) {
  ...
  var api_emi_plans = methods.emi_plans;
  if (methods.emi && api_emi_plans) {
    emi_banks.selected = methods.emi = false;
    each(emi_banks, function(code, bank) {
      var api_bank = api_emi_plans[code];
      if (api_bank && amount >= api_bank.min_amount) {
        bank.min_amount = api_bank.min_amount;
        bank.plans = api_bank.plans;
        methods.emi = true;
      } else {
        bank.plans = null;
      }
  ...
  if (
    !((emiMethod && methods.emi) || methods.card) || recurring || international
  ) {
    methods.emi = false;
  }
  ...
  wallets.sort(function(walletA, walletB) {
    return walletB.custom || session.offers.wallet[walletB];
  });

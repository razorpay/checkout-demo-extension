...
(function() {
	...
	Razorpay.prototype.createlightBox = function(template) {
        ...
        if (this.options.netbanking){
            $('.rzp-tabs li').click(function(){
                $('#' + this.getAttribute('data-target')).addClass('active').siblings('.active').removeClass('active')
	...
	Razorpay.prototype.handleAjaxResponse = function(response) {
		...
		if (response.http_status_code != 200 && response.error)
		{
			...
			var autosubmitformTemplate = Razorpay.templates.autosubmit;
			var div = document.createElement('div');
	...
	Razorpay.prototype.preHandler = function() {
		...
		this.createlightBox(Razorpay.templates.modal);
	};
